...
## 2. Murano Product Configuration Requirements
...
2.  Translate that information from machine-readable and terse input to Murano, back into contextual and human readable data ready to be taken into RCM - i.e. a signal object
...
The configuration below wraps a user-defined value with a ${...}, and other names/keys are meant to be an exact match that will be used by RCM.  Some values for a key are filled in with example strings - noted with an “e.g.”.
...
## 4. Device Data Transport Schema
...
### 5.1 	State Representation Types
...
Type Key Name|Channel Property Name|Property Required?
--|--|--
BINARY|value_mapping|Yes

**ExoSense Status:** Not Supported

**Description**

An integer value - typically 1 or 0 sent from devices, OR exact match string.

**Acceptable Values**

Any string, or from list of common enumerated types: 
*  ON  
*  OFF
*  UP
*  DOWN
...
Type Key Name|Channel Property Name|Property Required?
--|--|--
CATEGORICAL|value_mapping|Yes

**ExoSense Status:** Not Supported

**Description**

Integer value, or exact match string.

Future: Evaluate if it is useful to have translated words for common multi-state values - e.g.
-   ON
-   OFF
-   UP
-   DOWN
...
```
### 5.2 	Real Number & String Types
Many of these types will represent base physical measurements (temperature, length, etc), or derived measurements (velocity), as noted in this [Wikipedia article](https://en.wikipedia.org/wiki/List_of_physical_quantities), but there are some straight numeric types as well such as a percentage and a generic numeric type that could be anything that is dimensionless.

There is one non-numeric type - a generic “String” which can be any string value, and is unitless - similar to the “Number” type.

#### Type: String (unitless)
